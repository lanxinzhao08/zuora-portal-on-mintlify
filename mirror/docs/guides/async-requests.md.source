---
seo:
  title: Asynchronous requests - Zuora
  description: Describes how to make async calls to the Zuora API
  keywords: async, async calls, async requests, async API
markdown:
  toc:
    hide: false
redirects:
  /rest-api/general-concepts/async-requests/: {}
---
...
Most API endpoints documented in the [API Reference](/api-references/api/overview/) process requests synchronously. In high-throughput scenarios, your requests to these endpoints are usually rate limited.
...
You can make asynchronous requests to the POST, PUT, and DELETE operations (except [Actions](/v1-api-reference/api/actions/) for all resources documented in the [API Reference](/api-references/api/overview/).
...
- The asynchronous API framework is an addition to the following previously introduced operations that support asynchronous request processing. These operations do not work with the **Async Request Succeeded** and **Async Request Failed** standard events referenced below.
  ...
  - [Create a job to hard delete billing document files](/api-references/api/operation/POST_BillingDocumentFilesDeletionJob)
  - [Cancel a journal run](/api-references/api/operation/PUT_JournalRun)
  - [Run trial balance](/api-references/api/operation/PUT_RunTrialBalance)
...
- Ensure that you are familiar with corresponding synchronous API operations documented in the [API Reference](/api-references/api/overview/), and the <a href="https://docs.zuora.com?resourceId=platform-events-and-notifications-overview" target="_blank">Events and Notifications</a> feature of Zuora.
...
To create callout notifications through the API, use the [Create a notification definition](/api-references/api/operation/POST_Create_Notification_Definition) operation and specify the following required fields and other conditional or optional fields as needed:
...
The POST, PUT, and DELETE operations (except [Actions](/v1-api-reference/api/actions/)) for all resources documented in the [API Reference](/api-references/api/overview/) support asynchronous request processing.
